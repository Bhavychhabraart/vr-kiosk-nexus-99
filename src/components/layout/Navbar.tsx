...
import { Home, Grid, Settings, ChevronRight, Menu, X } from "lucide-react";
const Navbar = () => {
  ...
  return <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "bg-vr-dark/90 backdrop-blur-lg shadow-md" : "bg-transparent"}`}>
      ...
      {/* Mobile Navigation */}
      {isMenuOpen && <div className="md:hidden absolute top-full left-0 right-0 bg-vr-dark/95 backdrop-blur-md border-b border-vr-primary/20 py-2 z-50">
...
const NavItem = ({
  ...
  return <Link to={to} className={`flex items-center gap-1 px-4 py-2 rounded-md transition-colors relative font-medium ${active ? "text-vr-secondary bg-vr-primary/10" : "text-vr-text/80 hover:text-vr-secondary hover:bg-vr-primary/5"}`}>
      <span className="text-vr-secondary">{icon}</span>
      <span>{label}</span>
      {active && <span className="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-12 h-0.5 bg-vr-secondary rounded-full" />}
    </Link>;
};
...
const MobileNavItem = ({
  ...
  return <Link to={to} className={`flex items-center justify-between px-4 py-3 border-b border-vr-primary/10 ${active ? "bg-vr-primary/10 text-vr-secondary" : "text-vr-text/80 hover:text-vr-secondary"}`} onClick={onClick}>
