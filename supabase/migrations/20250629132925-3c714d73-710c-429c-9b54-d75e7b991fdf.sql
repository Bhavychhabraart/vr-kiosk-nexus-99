...
-- Setup vrarcadia002@gmail.com user as machine admin with venue and games
DO $$
DECLARE
    target_user_id UUID;
    new_venue_id UUID;
    venue_name TEXT := 'vrarcadia002''s VR Arcade';
    machine_serial TEXT;
    games_assigned INTEGER := 0;
    game_record RECORD;
    existing_venue_id UUID;
BEGIN
    -- Get the user ID for vrarcadia002@gmail.com from profiles table
    SELECT id INTO target_user_id
    FROM profiles
    WHERE email = 'vrarcadia002@gmail.com';
    
    -- If user doesn't exist in profiles, we need to find them in auth.users
    IF target_user_id IS NULL THEN
        RAISE NOTICE 'User vrarcadia002@gmail.com not found in profiles table. Please ensure they have signed up first.';
        RETURN;
    END IF;
    ...
    RAISE NOTICE 'Successfully set up user vrarcadia002@gmail.com as machine admin';
